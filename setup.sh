...
TAILSCALEIP=$(tailscale ip -4 homelab)
...
iptables --in-interface eth0 --append PREROUTING --table nat --protocol tcp --destination $MYIP --dport 1:65535 --jump DNAT --to-destination $TAILSCALEIP
iptables --out-interface tailscale0 --append POSTROUTING --table nat --protocol tcp --destination $TAILSCALEIP --dport 1:65535 --jump SNAT --to-source $MYIP
sudo iptables -t nat -A POSTROUTING -p tcp --dport 1:65535 -j MASQUERADE
iptables --in-interface eth0 --append PREROUTING --table nat --protocol udp --destination $MYIP --dport 1:65535 --jump DNAT --to-destination $TAILSCALEIP
iptables --out-interface tailscale0 --append POSTROUTING --table nat --protocol udp --destination $TAILSCALEIP  --dport 1:65535 --jump SNAT --to-source $MYIP
sudo iptables -t nat -A POSTROUTING -p udp --dport 1:65535 -j MASQUERADE
